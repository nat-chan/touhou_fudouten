...
################ 補助関数群 ################
def hatafast(p, z=np.array([[1, 1, 1]])) -> np.ndarray:
    """畑写像を高速に計算"""
    A0 = [np.array([
...
class Reimu:
    ...
    def update(self, t: int, controller_input, is_hit: bool) -> None:
        """自機の位置、無敵等をupdate"""
    ...
    def draw(self, t: int, screen: pg.Surface):
        """Surfaceへの自機の描画"""
...
class GameStep:
    def __init__(self, screen_display, clock) -> None:
        ...
        if beats[0] != beats[1]:
            for i in range(3):
                if beats.ignite(0,i,0,0):
                    self.bullets += [StraightBullet(self.lc, circ((0,0),i*2*np.pi/self.way,1))for i in range(32)]
                if beats.ignite(2,i,0,0):
                    self.bullets += [StraightBullet(self.rc, circ((0,0),i*2*np.pi/self.way,1))for i in range(32)]
